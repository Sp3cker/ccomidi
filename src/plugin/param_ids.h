...
{
    return kFirstRowParam + static_cast<clap_id>(row * kParamsPerRow + static_cast<std::uint32_t>(slot));
}
...
inline bool decode_param_id(clap_id id, ParamAddress *address)
{
    ...
    const std::uint32_t row = static_cast<std::uint32_t>(adjusted / kParamsPerRow);
    const std::uint32_t slot = static_cast<std::uint32_t>(adjusted % kParamsPerRow);
