...
void append_preview_triplet(std::string *preview,
                            std::uint8_t channel,
                            ...
                            std::uint8_t value)
{
    if (!preview)
        return;
    char buffer[32];
    std::snprintf(buffer, sizeof(buffer), "B%X %02X %02X", channel, controller, value);
    if (!preview->empty())
        preview->append("  ");
...
std::string preview_for_row(const UiSnapshot &snapshot, std::size_t row)
{
    ...
    std::string preview = "Ch " + std::to_string(static_cast<unsigned>(channel + 1)) + ": ";
    switch (type) {
...
void draw_parameter_controls(EditorState *editor,
                             std::size_t row,
                             CommandType type,
                             const UiRowSnapshot &rowSnapshot)
{
    ...
    if (activeFields == 1) {
        float value = static_cast<float>(rowSnapshot.values[0]);
        const std::string label = std::string(field_label(type, 0)) + "##v" + std::to_string(row) + "_0";
        ImGui::SetNextItemWidth(-1.0f);
        if (ImGui::SliderFloat(label.c_str(), &value, 0.0f, 127.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp)) {
    ...
    const float controlWidth = (fullWidth - (spacing * static_cast<float>(activeFields - 1))) /
    ...
    for (int field = 0; field < activeFields; ++field) {
        ...
        float value = static_cast<float>(rowSnapshot.values[field]);
...
void draw_editor(EditorState *editor)
{
...
#ifdef __APPLE__
    auto *metalContext = static_cast<PuglMetalContext *>(puglGetContext(editor->view));
    ...
    ImGui::SetNextWindowSize(ImVec2(static_cast<float>(editor->width), static_cast<float>(editor->height)));
    ...
    if (ImGui::SliderInt("Output Channel", &outputChannel, 1, 16))
        apply_ui_param_change(editor->plugin, kParamOutputChannel, static_cast<double>(outputChannel - 1));

    ImGui::Spacing();
    if (ImGui::BeginTable("rows", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY)) {
        ...
        ImGui::TableSetupColumn("Preview", ImGuiTableColumnFlags_WidthStretch, 2.8f);
        ...
        for (std::size_t row = 0; row < kMaxCommandRows; ++row) {
            ...
            if (ImGui::Checkbox(("##enabled" + std::to_string(row)).c_str(), &enabled))
                apply_ui_param_change(editor->plugin,
                                      row_param_id(static_cast<std::uint32_t>(row), RowParamSlot::Enabled),
                                      enabled ? 1.0 : 0.0);
            ...
            if (ImGui::BeginCombo(("##type" + std::to_string(row)).c_str(), command_type_name(type))) {
                for (int candidate = static_cast<int>(CommandType::None);
                    ...
                    if (ImGui::Selectable(command_type_name(static_cast<CommandType>(candidate)), selected)) {
                        apply_ui_param_change(editor->plugin,
...
#else
    glViewport(0, 0, static_cast<int>(editor->width), static_cast<int>(editor->height));
...
PuglStatus on_pugl_event(PuglView *view, const PuglEvent *event)
{
    ...
    case PUGL_BUTTON_PRESS:
        ...
        break;
    case PUGL_KEY_PRESS:
        if (editor->embedded && is_plain_space_key(event) && !ImGui::GetIO().WantTextInput)
...
EditorState *editor_create(Plugin *plugin)
{
    ...
    puglSetSizeHint(editor->view, PUGL_DEFAULT_SIZE, kDefaultWidth, kDefaultHeight);
...
bool editor_set_parent(EditorState *editor, std::uintptr_t nativeParent)
{
    ...
    if (puglRealize(editor->view) != PUGL_SUCCESS)
        return false;
...
bool editor_show(EditorState *editor)
{
    ...
    puglShow(editor->view, editor->embedded ? PUGL_SHOW_PASSIVE : PUGL_SHOW_RAISE);
    return true;
}
