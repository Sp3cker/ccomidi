...
public:
    struct EncodedCommand {
        std::array<std::pair<std::uint8_t, std::uint8_t>, kMaxCommandMessages> messages = {};
        std::uint8_t count = 0;
        bool valid = false;
    };
    ...
    double row_value_raw(std::size_t channel, std::size_t row, std::size_t field) const;
    ...
    void emit_preapplied_changes(bool transportIsPlaying,
                                 bool channelChanged,
...
private:
    ...
    static std::uint8_t floor_to_u8(double value, std::uint8_t minValue, std::uint8_t maxValue);
    ...
    bool apply_event(const AutomationEvent &event, bool *channelChanged, std::array<bool, kMaxCommandRows> *rowChanged);
